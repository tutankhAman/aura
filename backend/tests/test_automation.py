...
class TestAutomationAgent(unittest.TestCase):
    ...
    def test_open_application_with_path(self, mock_exists):
        ...
        result = self.agent.open_application("TestApp", "C:\\Program Files\\TestApp\\app.exe")
    ...
    @patch('os.path.exists')
    def test_open_application_with_name(self, mock_exists):
        ...
        # Call the method under test
        result = self.agent.open_application("TestApp")
    ...
    @patch('os.startfile')
    @patch('os.path.exists')
    def test_open_file_default_app(self, mock_exists, mock_startfile):
        # Test opening file with default application
        mock_exists.return_value = True

        result = self.agent.open_file("C:\\path\\to\\file.txt")
        
        self.assertTrue(result)
        mock_startfile.assert_called_once_with("C:\\path\\to\\file.txt")

    @patch('os.path.exists')
    def test_open_file_specific_app(self, mock_exists):
        ...
        # Call the method under test
        result = self.agent.open_file("C:\\path\\to\\file.txt", "TestApp")
    ...
    def test_open_workspace(self, mock_open_file, mock_open_app):
        ...
        result = self.agent.open_workspace(workspace)
    ...
    def test_wait_for_window(self, mock_get_window):
        # Test waiting for window to appear
        mock_window = MagicMock()
        mock_get_window.side_effect = [None, None, mock_window]

        result = self.agent.wait_for_window("Test Window", timeout=2)
        
        self.assertEqual(result, mock_window)
        self.assertEqual(mock_get_window.call_count, 3)

    @patch('os.path.exists')
    def test_open_figma(self, mock_exists):
        ...
        result = self.agent.open_application("Figma")
        self.assertTrue(result)
        ...
        result = self.agent.open_application("Figma", expected_figma_path)
        self.assertTrue(result)
        self.mock_application.assert_called_once_with(backend="uia")
        mock_app_instance.start.assert_called_once_with(expected_figma_path)
        ...
        mock_window = MagicMock()
        self.mock_find_windows.return_value = [mock_window]
        window = self.agent.get_window_by_title("Figma")
